...
export default function Settings() {
  ...
  return (
    <div className="p-4 pb-28 space-y-3 relative z-10">
      <h1 className="text-lg font-extrabold">⚙️ الإعدادات</h1>

      <div className="glass p-4 flex items-center gap-3">
        <div className="flex">
          <span className="w-10 h-10 rounded-full flex items-center justify-center font-bold border-2 border-night-950 z-10"
            style={{ background: 'linear-gradient(135deg,#6a55b5,#544294)' }}>{myName.charAt(0)}</span>
          <span className="w-10 h-10 rounded-full flex items-center justify-center font-bold border-2 border-night-950 -mr-2"
            style={{ background: 'linear-gradient(135deg,#b0568f,#94427a)' }}>{partnerName.charAt(0)}</span>
        </div>
        <div className="flex-1">
          <div className="font-extrabold text-sm">{myName} ❤ {partnerName}</div>
      ...
      <div className="glass p-4">
        <div className="text-sm font-extrabold mb-3">🏅 الأوسمة ({toArabicNum(earned.size)} / {toArabicNum(BADGES.length)})</div>
        <div className="grid grid-cols-3 gap-2">
          {BADGES.map(b => {
            const got = earned.has(b.key)
            return (
              <div key={b.key} className={`rounded-2xl p-2.5 text-center border ${got ? 'border-gold/50 bg-gold/10' : 'border-lavender/15 bg-white/3 opacity-45'}`}>
                <div className="text-xl mb-0.5">{got ? b.icon : '🔒'}</div>
      ...
      <div className="glass p-4 text-xs space-y-1.5 text-lavender/80">
        <div className="text-sm font-extrabold text-ink mb-2">📊 أرقامكم</div>
        ...
        <div className="flex justify-between"><span>أفضل سلسلة</span><b>🔥 {toArabicNum(game?.bestStreak ?? 0)}</b></div>
      </div>

      {/* الخصوصية */}
      <div className="glass p-4">
        <div className="text-sm font-extrabold mb-1">🔒 قفل التطبيق (PIN)</div>
      ...
      {pinModal && (
        <Modal onClose={() => setPinModal(false)}>
          <div className="text-lg font-extrabold mb-3">🔒 تعيين رقم سري</div>
