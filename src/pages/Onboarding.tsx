...
export default function Onboarding() {
  ...
  if (!user) {
    return (
      <Shell>
        <h2 className="text-xl font-extrabold mb-1">{mode === 'signup' ? 'أهلًا فيكم 👋' : 'رجعتوا لنا 🌙'}</h2>
  ...
  if (!profile?.coupleId) {
    return (
      <Shell>
        <h2 className="text-xl font-extrabold mb-1">نربط الميثاق؟ 💞</h2>
  ...
  return (
    <Shell>
      <h2 className="text-xl font-extrabold mb-1">باقي طرف واحد 💫</h2>
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-dvh flex items-center justify-center p-6 relative z-10">
      <div className="glass p-6 w-full max-w-sm anim-slideup">
        <div className="text-center mb-5">
          <div className="text-4xl mb-2">🌙</div>
          <div className="gold-glow text-3xl font-extrabold">ميثاق</div>
          <div className="text-xs text-lavender/60 mt-1">تطبيقنا الخاص — لنا نحن الاثنين فقط</div>
        </div>
        {children}
      </div>
    </div>
