  ...
  return (
    <div className="glass p-5">
      <div className="flex justify-between text-xs text-lavender/60 mb-4">
        <span>نقاطك: {toArabicNum(s.scores[uid] ?? 0)} ⭐</span>
      ...
      {s.phase === 'judge' && card && (
        <>
          <div className="glass p-5 text-center anim-popin mb-4"
            style={{ borderColor: card.kind === 'dare' ? 'rgba(240,217,168,.55)' : 'rgba(122,92,201,.55)' }}>
            <div className="text-[10px] text-lavender/60 mb-2">{card.kind === 'dare' ? '🔥 تحدي' : '💬 صارحني'} — على {myDraw ? 'ك أنت' : ` ${partnerName}`}</div>
          ...
          {myDraw ? (
            ...
            <>
              <p className="text-center text-sm mb-3">أنت الحَكَم — {partnerName} {card.kind === 'dare' ? 'نفّذ التحدي؟' : 'جاوب بصدق وقنعك؟'} 🧑‍⚖️</p>
              <div className="grid grid-cols-2 gap-3">
